...
/// Runtime port of the level generator's shape masks (gen_all_levels.dart).
/// Given a level's shapeName + gridSize it rebuilds the exact silhouette,
/// used by the "Set It Free" completion reveal.

const Map<String, String> _displayToId = {
  'Circle': 'circle', 'Square': 'square', 'Triangle': 'triangle',
  'Diamond': 'diamond', 'Cross': 'plus', 'Ring': 'ring', 'Hexagon': 'hexagon',
  'Hourglass': 'hourglass', 'Moon': 'moon', 'Sun': 'sun', 'House': 'house',
  'Pine Tree': 'tree', 'Cup': 'cup', 'Kite': 'kite', 'Shield': 'shield',
  'Bell': 'bell', 'Heart': 'heart', 'Star': 'star', 'Fish': 'fish',
  'Light Bulb': 'bulb', 'Butterfly': 'butterfly', 'Rocket': 'rocket',
  'Flower': 'flower', 'Gem': 'gem', 'Crown': 'crown', 'Trophy': 'trophy',
  'Snowflake': 'snowflake', 'Aeroplane': 'plane', 'Umbrella': 'umbrella',
  'Target': 'target', 'Anchor': 'anchor', 'Infinity': 'infinity',
  'Mushroom': 'mushroom', 'Music Note': 'note', 'Cat': 'cat', 'Key': 'key',
  'Sailboat': 'boat', 'Leaf': 'leaf', 'Lightning': 'zap', 'Clover': 'clover',
  ...
  'Ice Cream': 'icecream', 'Pinwheel': 'pinwheel', 'Whale': 'whale',
  'Candy': 'candy', 'Cactus': 'cactus', 'Camera': 'camera',
  'Rabbit': 'rabbit', 'Flame': 'flame', 'Turtle': 'turtle', 'Lock': 'lock',
  'Cupcake': 'cupcake', 'Arrow': 'arrow', 'Paw Print': 'paw',
  'Guitar': 'guitar', 'Raindrop': 'drop', 'Skull': 'skull',
  'Pentagon': 'pentagon', 'Bone': 'bone',
  'Elephant': 'elephant', 'Yin Yang': 'yinyang', 'Palm Tree': 'palm',
  'Gear': 'gear', 'Penguin': 'penguin', 'Saturn': 'saturn',
  'Goblet': 'wineglass', 'Dove': 'dove', 'Spiral': 'spiral',
  ...
  'Top Hat': 'tophat', 'Crab': 'crab', 'Volcano': 'volcano',
  'Scissors': 'scissors', 'Seahorse': 'seahorse', 'Hexagram': 'hexagram',
  'Open Book': 'book', 'Bee': 'bee', 'Ocean Wave': 'wave',
  'Envelope': 'envelope', 'Jellyfish': 'jellyfish', 'Tornado': 'tornado',
  'Diamond Ring': 'diamondring', 'Swan': 'swan', 'Hot Air Balloon': 'hotair',
  'Bow Tie': 'bowtie', 'Fox': 'fox', 'Windmill': 'windmill',
  'T-Shirt': 'tshirt', 'Mouse': 'mouse', 'Lighthouse': 'lighthouse',
  'Boot': 'boot', 'Chick': 'chick', 'Cherries': 'cherry',
  'Lantern': 'lantern', 'Serpent': 'serpent', 'Acorn': 'acorn',
  'Dumbbell': 'dumbbell', 'Dinosaur': 'dino', 'Seashell': 'shell',
  'Flag': 'flag', 'Ladybird': 'ladybird', 'Starburst': 'star8',
  'Kettlebell': 'kettlebell', 'Spider': 'spider', 'Keyhole': 'keyhole',
  'Question Mark': 'question', 'Dog': 'dog', 'Trinity Rings': 'rings3',
  'Exclamation': 'exclaim', 'Bear': 'bear', 'Wrench': 'wrench',
...
/// How the freed shape "comes alive" in the reveal.
/// flyAway | beat | bloom | shine | swing | float
String archetypeFor(String displayName) {
  switch (displayName) {
    case 'Butterfly':
    case 'Fish':
    case 'Rocket':
...
/// Rebuilds the mask for a level's shape. Empty set if the name is unknown.
Set<String> maskForShape(String displayName, int gs) {
  final id = _displayToId[displayName];
  if (id == null) return {};
  final mask = <String>{};
  final mid = gs / 2;
  for (int y = 0; y < gs; y++) {
    for (int x = 0; x < gs; x++) {
      ...
      if (_inShape(id, px, py)) mask.add('$x,$y');
    }
  }
  return mask;
}

bool _inShape(String shape, double px, double py) {
  ...
  switch (shape) {
    ...
    case 'plane':
      ...
      final wing = ax <= 0.88 && py > ax * 0.35 - 0.05 && py < ax * 0.35 + 0.18;
      final tail = ax <= 0.40 && py > 0.55 + ax * 0.30 - 0.04 && py < 0.55 + ax * 0.30 + 0.12;
      return wing || tail;
    case 'umbrella':
    ...
    case 'target':
      return r <= 0.28 || (r >= 0.46 && r <= 0.64) || (r >= 0.80 && r <= 0.95);
    case 'anchor':
